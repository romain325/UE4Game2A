...
void AProjetVR2ACharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("ResetVR", IE_Pressed, this, &AProjetVR2ACharacter::OnResetVR);

	// Bind movement events
	PlayerInputComponent->BindAxis("MoveForward", this, &AProjetVR2ACharacter::MoveForward);
	PlayerInputComponent->BindAxis("MoveRight", this, &AProjetVR2ACharacter::MoveRight);

	// We have 2 versions of the rotation bindings to handle different kinds of devices differently
	// "turn" handles devices that provide an absolute delta, such as a mouse.
	// "turnrate" is for devices that we choose to treat as a rate of change, such as an analog joystick
	PlayerInputComponent->BindAxis("Turn", this, &APawn::AddControllerYawInput);
	PlayerInputComponent->BindAxis("TurnRate", this, &AProjetVR2ACharacter::TurnAtRate);
...
void AProjetVR2ACharacter::OnResetVR()
{
	UHeadMountedDisplayFunctionLibrary::ResetOrientationAndPosition();
}

void AProjetVR2ACharacter::BeginTouch(const ETouchIndex::Type FingerIndex, const FVector Location)
{
...
void AProjetVR2ACharacter::EndTouch(const ETouchIndex::Type FingerIndex, const FVector Location)
{
...
//void AProjetVR2ACharacter::TouchUpdate(const ETouchIndex::Type FingerIndex, const FVector Location)
//{
...
void AProjetVR2ACharacter::MoveForward(float Value)
{
	if (Value != 0.0f)
	{
...
void AProjetVR2ACharacter::MoveRight(float Value)
{
	if (Value != 0.0f)
	{
		// add movement in that direction
...
void AProjetVR2ACharacter::TurnAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds());
}

void AProjetVR2ACharacter::LookUpAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerPitchInput(Rate * BaseLookUpRate * GetWorld()->GetDeltaSeconds());
}

bool AProjetVR2ACharacter::EnableTouchscreenMovement(class UInputComponent* PlayerInputComponent)
{
	if (FPlatformMisc::SupportsTouchInput() || GetDefault<UInputSettings>()->bUseMouseForTouch)
	{
		PlayerInputComponent->BindTouch(EInputEvent::IE_Pressed, this, &AProjetVR2ACharacter::BeginTouch);
		PlayerInputComponent->BindTouch(EInputEvent::IE_Released, this, &AProjetVR2ACharacter::EndTouch);

		//Commenting this out to be more consistent with FPS BP template.
		//PlayerInputComponent->BindTouch(EInputEvent::IE_Repeat, this, &AProjetVR2ACharacter::TouchUpdate);
		return true;
	}
